...
3. **是否指定思考强度（`--effort`）？**——**默认否**：`↑`/`↓`（或 `←`/`→`）在 `默认 / low / medium / high / xhigh / max` 之间移动，`enter` 确认选中项；也可以直接按 `1`-`5` 直选对应档位，或按 `n` 快速保持默认并继续；
4. **是否命名（`--name`）？**——**默认否**：`n` / `enter` 跳过；选 `y`，输入名字，再按 `enter` 确认（空 = 不命名）。

向导完成后，本工具会：

- 在会话的**目录**（cwd）里打开一个新终端（macOS = Terminal，Windows = PowerShell）；
- 运行 `claude [--dangerously-skip-permissions] [--effort <档位>] [--name <名字>] '<指令>'`——启动一个**全新的** Claude 会话（**不带 `--resume`**）；你选择的开关会追加在 `claude` 之后、指令之前；
